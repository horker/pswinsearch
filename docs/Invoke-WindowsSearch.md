...
## SYNOPSIS
Searches the computer using the Windows Search Service.
...
## DESCRIPTION
The cmdlet makes a query to the Windows Search Service and gets results as PowerShell objects.
...
By specifying the -Verbose parameter, you can see the SQL generated by the cmdlet internally. It is useful to make sure what query is actually made when you make a complicated query or get unexpected results.
...
Note that the cmdlet searches only the files indexed by the Windows Search Service. This behavior is different from that of Windows Explorer.
